...
/**
 * A class that represents a GiantCrab -> a type of enemy.
 * @author Harshath Muruganantham
 * @see SeaCreature
 * @see IRune
 */
public class GiantCrab extends SeaCreature implements IRune {
    ...
    public GiantCrab() {
        ...
        RuneManager.getInstance().storeRuneAmount(this, RandomNumberGenerator.getRandomInt(318, 4961));
    }
            ...
            if(action != null){
                if (getBehaviours().containsKey(998)) {
                    ...
                    if (RandomNumberGenerator.getChance(50)) {  // i didnt do this inside the slam method because i would have to return null and do an if statement here anyways
                        action = new SlamAllAction();      // and its better to do it here because inside this conditional, it is an intrinsic attack
                    }
                }
                return action;}
        }
