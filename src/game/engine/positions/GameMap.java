package game.engine.positions;

import game.engine.actors.Actor;
import game.engine.actors.ActorLocationsIterator;
	...
	public void tick() {
		...
		for (Actor actor : actorLocations) {
			if (this.contains(actor)) {
				...
				// in case if a weapon has a passive special ability (e.g. drop fire at every turn without the actor having to execute the action)
				for (WeaponItem weapon : new ArrayList<WeaponItem>(actor.getWeaponInventory())) {
					weapon.tick(actorLocations.locationOf(actor), actor);
				}
			}
		}
